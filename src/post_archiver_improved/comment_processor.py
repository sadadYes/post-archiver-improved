...
class CommentProcessor:
    ...
    def __init__(self, api_client: Any, comment_extractor: Any) -> None:
        """
    ...
    def extract_comments(
        ...
        try:
            ...
            while continuation_token and len(comments) < max_comments:
                try:
                    logger.debug(
                        f"Fetching comment batch (comments so far: {len(comments)})"
                    )
                    comment_response = self.api.get_continuation_data(
                        continuation_token
                    )
                    ...
                    logger.debug(
                        f"Extracted {len(new_comments)} comments in this batch"
                    )
            ...
            logger.info(
                f"Successfully extracted {len(comments)} comments for post {post_id}"
            )
        ...
        try:
            ...
            for tab in tabs:
                ...
                for section in sections:
                    ...
                    if (
                        section_renderer.get("sectionIdentifier")
                        == "comment-item-section"
                    ):
                        for item in section_renderer.get("contents", []):
                            if "continuationItemRenderer" in item:
                                continuation_endpoint = item[
                                    "continuationItemRenderer"
                                ]["continuationEndpoint"]
                                token = continuation_endpoint["continuationCommand"][
                                    "token"
                                ]
                                logger.debug(
                                    f"Found comment continuation token: {token[:20]}..."
                                )
                                return str(token)
        ...
        try:
            ...
            if "frameworkUpdates" in response:
                ...
                if "entityBatchUpdate" in framework_updates:
                    mutations = framework_updates["entityBatchUpdate"].get(
                        "mutations", []
                    )
            ...
            if "onResponseReceivedEndpoints" in response:
                for endpoint in response["onResponseReceivedEndpoints"]:
                    ...
                    for item in items:
                        comment = self._process_comment_item(
                            item, entity_payloads, max_replies_per_comment
                        )
                        if comment:
        ...
        try:
            if "commentThreadRenderer" in item:
                ...
                # Check for old format (comment.commentRenderer)
                    ...
                    "comment" in thread_renderer
                    and "commentRenderer" in thread_renderer["comment"]
                ):
                    return self._process_old_format_comment(
                        thread_renderer, max_replies_per_comment
                    )

            elif "commentRenderer" in item:
                comment_result = self.extractor.extract_comment_from_renderer(
                    item["commentRenderer"]
                )
                return comment_result  # type: ignore
    ...
    def _process_new_format_comment(
        ...
        try:
            ...
            for entity in entity_payloads:
                ...
                elif entity_key.startswith(
                    inline_replies_key
                ) and "commentEntityPayload" in entity.get("payload", {}):
            ...
            if "commentRepliesRenderer" in replies_data:
                self._extract_replies_from_renderer(
                    replies_data["commentRepliesRenderer"],
                    comment,
            ...
            if "continuations" in replies_renderer:
                for continuation in replies_renderer["continuations"]:
                    if "nextContinuationData" in continuation:
                        token = continuation["nextContinuationData"].get(
                            "continuation", ""
                        )
                        if token:
                            return str(token)
                        ...
                        if (
                            "command" in button
                            and "continuationCommand" in button["command"]
                        ):
                            token = button["command"]["continuationCommand"].get(
                                "token", ""
                            )
                            if token:
                                return str(token)
    ...
    def _fetch_replies_from_continuation(
        ...
        try:
            while (
                current_token
                and len(replies) < max_replies
                and fetch_attempts < max_attempts
            ):
                fetch_attempts += 1
                logger.debug(
                    f"Fetching reply batch {fetch_attempts} (replies so far: {len(replies)})"
                )
                ...
                try:
                    ...
                    logger.debug(
                        f"Fetched {len(batch_replies)} replies in batch {fetch_attempts}"
                    )

                    # Look for next continuation token
                    current_token = self._find_reply_continuation_token_in_response(
                        response
                    )
            ...
            if "frameworkUpdates" in response:
                ...
                if "entityBatchUpdate" in framework_updates:
                    ...
                    entity_payloads = [
                        ...
                        for m in mutations
                        if "payload" in m
                        and "commentEntityPayload" in m.get("payload", {})
                    ]
        ...
        try:
                ...
                for endpoint in response["onResponseReceivedEndpoints"]:
                    ...
                    for item in items:
                        if "continuationItemRenderer" in item:
                            ...
                            continuation_endpoint = continuation_renderer.get(
                                "continuationEndpoint", {}
                            )
                            if "continuationCommand" in continuation_endpoint:
                                token = continuation_endpoint[
                                    "continuationCommand"
                                ].get("token", "")
                                if token:
                                    return str(token)
                            ...
                            if "button" in continuation_renderer:
                                ...
                                if "buttonRenderer" in button:
                                    ...
                                    if "continuationCommand" in command:
                                        token = command["continuationCommand"].get(
                                            "token", ""
                                        )
                                        if token:
                                            return str(token)
            ...
            if "continuationContents" in response:
                ...
                if "commentRepliesContinuation" in continuation_contents:
                    replies_continuation = continuation_contents[
        ...
        try:
            if "onResponseReceivedEndpoints" in response:
                for endpoint in response["onResponseReceivedEndpoints"]:
                    ...
                    for item in items:
                        if "continuationItemRenderer" in item:
                            continuation_endpoint = item["continuationItemRenderer"][
                                "continuationEndpoint"
                            ]
                            token = continuation_endpoint["continuationCommand"][
                                "token"
