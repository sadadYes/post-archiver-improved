...
from post_archiver_improved.models import ArchiveData, Post, Comment, Author
from post_archiver_improved.exceptions import (
    APIError, ParseError, ValidationError, NetworkError
)
...
class TestCommunityPostScraper:
    ...
    def test_scraper_initialization_no_comments(self):
        ...
        config = Config(
            scraping=ScrapingConfig(extract_comments=False),
            output=OutputConfig()
        )
...
class TestScrapePosts:
    ...
    def test_scrape_posts_basic_success(self, mock_extractor_class, mock_api_class, mock_validate):
    ...
    def test_scrape_posts_timing_metadata(self, mock_datetime, mock_validate):
        ...
        with patch('post_archiver_improved.scraper.YouTubeCommunityAPI') as mock_api_class:
            mock_api = Mock()
            mock_api.get_initial_data.return_value = {
...
class TestPostProcessing:
    ...
    def test_process_posts_batch_basic(self, mock_extractor_class):
        ...
        contents = [
            {
                "backstagePostThreadRenderer": {
                    "post": {
                        "backstagePostRenderer": {"postId": "post1"}
                    }
                }
            },
            {
        ...
        with patch.object(self.scraper, '_extract_post_comments') as mock_extract_comments:
    ...
    def test_process_posts_batch_with_limit(self):
        ...
        contents = [
            {
                "backstagePostThreadRenderer": {
                    "post": {
                        "backstagePostRenderer": {"postId": f"post{i}"}
                    }
                }
            }
            for i in range(10)
        ...
        with patch('post_archiver_improved.scraper.PostExtractor') as mock_extractor_class:
            mock_extractor = Mock()
            mock_posts = []
            for i in range(10):
                mock_post = Mock(spec=Post)
            ...
            posts = self.scraper._process_posts_batch(contents, "UC123456789", max_posts=3)
        ...
        contents = [
            {
                "backstagePostThreadRenderer": {
                    "post": {
                        "backstagePostRenderer": {"postId": "valid_post"}
                    }
                }
            },
            {
                "backstagePostThreadRenderer": {
                    "post": {
                        "invalidRenderer": {}  # This should cause an error
                    }
                }
            },
            {
                "backstagePostThreadRenderer": {
                    "post": {
                        "backstagePostRenderer": {"postId": "another_valid_post"}
                    }
                }
            }
        ]
...
class TestImageDownloading:
    ...
    def setup_method(self):
        """Set up test fixtures."""
        self.config = Config(
            scraping=ScrapingConfig(download_images=True),
            output=OutputConfig()
        )
        self.scraper = CommunityPostScraper(self.config)
...
class TestCommentExtraction:
    ...
    def setup_method(self):
        """Set up test fixtures."""
        self.config = Config(
            scraping=ScrapingConfig(extract_comments=True),
            output=OutputConfig()
        )
        self.scraper = CommunityPostScraper(self.config)
    ...
    def test_extract_post_comments_with_error(self):
        ...
        mock_comment_extractor = Mock()
        mock_comment_extractor.extract_comments.side_effect = APIError("Comment API failed")
        self.scraper.comment_extractor = mock_comment_extractor
...
class TestScraperIntegration:
    ...
    @patch('post_archiver_improved.scraper.YouTubeCommunityAPI')
    @patch('post_archiver_improved.scraper.PostExtractor')
    def test_full_scraping_workflow(self, mock_extractor_class, mock_api_class, mock_validate):
